...
export default function Provisioning() {
  ...
  const fetchProfiles = async () => {
    try {
      setLoading(true);
      const response = await enrollmentService.listProfiles();
      setProfiles(response.data);
    } catch (error) {
      toast({
        title: 'Erro de Conexão',
        description: 'Não foi possível carregar os perfis de provisionamento.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
            ...
            {profiles.map((profile) => (
              <div key={profile.id} className="card-glass border-border/40 p-6 hover:border-primary/40 transition-all group relative overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  ...
                  <div className="p-3 rounded-lg bg-secondary/30 border border-border/20">
                    <span className="block text-[9px] text-muted-foreground uppercase font-bold mb-1">Apps Permitidos</span>
                    <span className="text-xs font-semibold text-foreground">
                      {profile.allowed_apps.length} pacotes
                    </span>
                  </div>
                </div>
              </div>
      ...
      <Dialog open={isEditorOpen} onOpenChange={setIsEditorOpen}>
        <DialogContent className="sm:max-w-[500px] p-0 bg-background border-border/50 overflow-hidden shadow-2xl">
          ...
          <div className="p-6 space-y-4">
             ...
             <div className="space-y-1.5">
                <Label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Apps Permitidos (separados por vírgula)</Label>
                <Input 
                  placeholder="Ex: com.elion.app, com.google.android.calculator" 
                  value={formData.allowed_apps_str}
                  onChange={(e) => setFormData({ ...formData, allowed_apps_str: e.target.value })}
                  className="bg-card/50"
                />
             </div>
          ...
          <div className="flex items-center justify-end p-4 border-t border-border bg-muted/20 gap-2">
              ...
              <Button size="sm" onClick={handleSaveProfile} disabled={isSubmitting}>
                  {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Settings className="w-4 h-4 mr-2" />}
                  Salvar Perfil
              </Button>
          </div>
        </DialogContent>
